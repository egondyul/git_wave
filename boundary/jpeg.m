...
dx = fread(Fgrid,1,'double');
dz = fread(Fgrid,1,'double');
dt = fread(Fgrid,1,'double');
fclose(Fgrid);
...
%-----Rzz-------------%
%  field = 'R_zz_20';
%      frec = fopen([field '.txt'],'r'); % enter name of the file
%      REC = fread(frec,[Nx,inf],'double');
%      fclose(frec);
%     h=figure;
%     dz = dx;
%     nx=Nx;
...
if(flagRec==1)
    field = 'v_z_rec_1';
     ...
     REC2 = fread(frec2,[Nx,inf],'double');
     fclose(frec2);
 
     h=figure;
     %t_array=0:dt:time+3/freq+dt;
     [n,m]=size(REC);
     t_array=0:dt:m*dt-dt;
     plot(REC(1,:),t_array);
     hold on
     plot(REC2(1,:),t_array);
     saveas(h,'trassa', 'png');
...
if(flagSnapSigma==1)
    for k=1:9
        ...
        name=strcat(Name,num2str(k));
         ...
         nx=Nx;
         nz=Nz;
         if(strcmp(SnapName,'sigma_xz_')||strcmp(SnapName,'R_xz_'))
             nx=Nx-1;
             nz=Nz-1;
         end
    ...
    if(strcmp(SnapName,'sigma_xx_')||strcmp(SnapName,'sigma_zz_')||strcmp(SnapName,'sigma_xz_'))
            caxis([-2.5*10^(-8) 2.5*10^(-8)]) %for sigma
    end
...
REC=fread(frec,[Nx-1,inf],'double');
fclose(frec);
h=figure;
dz = dx;
